...
function Images() {
    ...
    const { 
        ...
        isValidating } = useSWR('/api/getImages', fetchImages, { revalidateOnFocus: false,
    });
    ...
    return (
        <div>
            <button 
                ...
                className="fixed bottom-10 right-10 bg-violet-400/90 text-white px-5 py-3 rounded-md hover:bg-violet-500 focus:outline-none focus:right-2 focus:ring-violet-400 font-bold z-20"
            >
                {!isLoading && isValidating ? "Refereshing..." : "Refresh Images"}
            </button>
            {isLoading && (
